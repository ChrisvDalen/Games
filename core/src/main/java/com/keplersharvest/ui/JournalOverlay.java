...
public final class JournalOverlay extends Overlay {
    ...
    public JournalOverlay(Skin skin, GameSession session, ReaderOverlay reader) {
        super(skin, "Evidence Journal", 800f, 480f);
    ...
    protected void rebuild() {
        ...
        page.add(new Label("Recovered logs  " + session.journal().foundLogCount()
                + " / " + session.journal().totalLogs(), skin, "accent")).left().padBottom(4f).row();
        ...
        for (CrewLogDefinition log : session.journal().foundLogsInOrder()) {
            ...
            page.add(open).left().fillX().padTop(3f).row();
        }

        page.add(new Label("What this adds up to", skin, "accent")).left().padTop(14f).padBottom(4f).row();
        ...
        for (EvidenceDefinition evidence : session.journal().recordedEvidence()) {
            ...
            text.setWrap(true);
            entry.add(text).left().width(690f).padTop(3f).row();
            entry.add(new Label("source: " + evidence.source(), skin, "muted")).left().padTop(4f);
            page.add(entry).left().fillX().padTop(6f).row();
        }

        page.add(buildConclusion()).left().fillX().padTop(16f).row();
    ...
    private Table buildConclusion() {
        ...
        if (logsShort > 0) {
            box.add(new Label(logsShort + " more crew log" + (logsShort == 1 ? "" : "s") + " to find.",
                    skin, "muted")).left().padTop(3f).row();
        }
        for (String missing : session.journal().missingForReveal()) {
            box.add(new Label("- " + missing, skin, "muted")).left().padTop(2f).row();
        }
        return box;
    }
}
